...
class w_user {
    ...
    protected function init_session() {
        ...
        if( $verify_sign === $client_sign ) {
            ...
            $row =$wdb->get("{$wconfig['db']['tablepre']}session", "*",
                array(
                "sid" => $client_sid
                )
             );
            ...
            if( !empty($row) && (($row['lastvisit'] + $session_expire) > W_TIMESTAMP) ) {
                ...
                $this->formtoken = $row['formtoken'];
                 ...
                 $wdb->update("{$wconfig['db']['tablepre']}session",
                     array(
                ...
                $guest_expire     = W_TIMESTAMP - 60;
                 ...
                 $wdb->delete("{$wconfig['db']['tablepre']}session",
                 array(
                     "OR"=>array(
                         "sid" => $client_sid,
                         "lastvisit" => $lastvisit_expire,
                         "AND" => array(
                             "uid" => '0',
                             "lastvisit[<]" =>  $guest_expire
                         )
                     )
                 )
                );
    ...
    protected function init_accesstoken() {
        ...
        $row =$wdb->get("{$wconfig['db']['tablepre']}accesstoken","*",
            array(
                "aid" =>$client_aid
            )
        ...
        if( !empty($row) && ($row['expire'] > W_TIMESTAMP) ) {
            ...
            $wdb->update("{$wconfig['db']['tablepre']}accesstoken",
                array(
                    "lastvisit" => W_TIMESTAMP
                ),
                array(
            ...
            $wdb->delete("{$wconfig['db']['tablepre']}accesstoken",
                array(
                    "expire[<]"=>W_TIMESTAMP
                )
            );
    ...
    public function init() {
        ...
        if( $this->uid > 0 ) {
            ...
            $user = $wdb->get("{$wconfig['db']['tablepre']}user",
                array(
                     "uid","username","roleid","point","balance","realname"
                ),
                array(
                    "uid"=>$this->uid
                )
    ...
    public function check_username($username) {
        ...
        $row = $wdb->get("{$wconfig['db']['tablepre']}user",
            array("uid"),
            array(
                "username"=>$username
            )
    ...
    public function register($username, $username_again, $password, $password_again, $email, $email_again, $roleid = 0, $mobile = '') {
        ...
        $last_user_id = $wdb->insert("{$wconfig['db']['tablepre']}user",
        array(
            "email" => $email,
            "username" =>$username,
            "mobile" => $mobile,
    ...
    public function login($username, $password, $mode = "session") {
        ...
        $result =$wdb->get("{$wconfig['db']['tablepre']}loginfailed", "*",
            array(
                "OR" => array(
                    "ipusername"=>$ip,
                    "ipusername"=>$username
        ...
        $user_row =$wdb->get("{$wconfig['db']['tablepre']}user", "*",
            array(
                $username_type => $username
            )
        );
        ...
        if( empty($user_row) ) {
            ...
            if( $ip_lock_arr['ipusername'] === '' ) {
                ...
                $wdb->update("{$wconfig['db']['tablepre']}loginfailed",
                    array(
                        "logintime" => W_TIMESTAMP,
                        "count[+]"=>1
                    ),
                    array(
                        "ipusername" => $ip
    ...
    public function check_access_permission() {
        ...
        $row =$wdb->get("{$wconfig['db']['tablepre']}role", "permission",
            array(
                "roleid" => $this->roleid
            )
        );
    ...
    public function get_curapp_user_menu() {
        ...
        if( $this->roleid == 1 ) {
            ...
            $row =$wdb->get("{$wconfig['db']['tablepre']}role", "*",
                array(
                    "roleid" => $this->roleid
                )
            );
    ...
    public function actionlog($loginfo) {
        if( $this->uid > 0 ) {
            ...
            $GLOBALS['wdb']->insert("{$log_table}",array(
