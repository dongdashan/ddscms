...
if( !isset($_POST['uid']) ) {
    ...
    $arr   = $wdb->select("{$wconfig['db']['tablepre']}role","roleid","status","rolename");
    $roles = array();
    foreach($arr as $k => $v) {
        if($v['status'] == 1) {
    ...
    $user = $wdb->get("{$wconfig['db']['tablepre']}user","*", array("uid"=>$uid));
    if( empty($user) ) {
    ...
    if( !empty($password) && !empty($salt) ) {
        ...
        $result=$wdb->update("{$wconfig['db']['tablepre']}user",
            array(
                "email"=>$email,
                "mobile"=>$mobile,
                "password"=>$password,
