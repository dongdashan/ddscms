...
if( !isset($_POST['name']) ) {
    ...
    include $wconfig['theme_path'] . '/admin/file/categoryupdate.html.php';
    ...
    $old = $wdb->select("{$wconfig['db']['tablepre']}upload_category",array(
        "cid", "pid", "name", "pic"
    ),array(
        "cid"=> $cid
    ));
    ...
    $result=$wdb->update("{$wconfig['db']['tablepre']}upload_category",array(
        "pid"=>$pid,
        "name"=>$name,
        "status"=>$status,
